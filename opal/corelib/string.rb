...
class ::String < `String`
  ...
  %x{
    ...
    function case_options_have_ascii(options) {
      if (options.length > 0) {
        for(const option of options) {
          switch (option) {
            case "ascii":
               ...
               break;
            case "fold":
              #{raise ArgumentError, ':fold not allowed for case swapping'};
              break; // linter happyness
            default:
  ...
  def *(count)
    %x{
      ...
      if (string.length * count >= Infinity) {
  ...
  def [](index, length = undefined)
    res = `slice(self, index, length)`
    return res.force_encoding(encoding) if res
    nil
  end
  ...
  def byteslice(index, length = undefined)
    ...
    return result.force_encoding(encoding) if result
    result
  end
  ...
  def capitalize(*options)
    %x{
      ...
      if (case_options_have_ascii(options) && first.codePointAt(0) > 255) return self;
  ...
  def downcase(*options)
    %x{
      if (case_options_have_ascii(options)) {
        return self.replace(/[A-Z]+/g, (match)=>{ return match.toLowerCase(); });
      }
  ...
  def each_grapheme_cluster(&block)
    return enum_for :each_grapheme_cluster unless block_given?

    clusters = `grapheme_segmenter().segment(self)`
    `for (const cluster of clusters) { #{yield `cluster.segment`}; }`
  ...
  def upcase(*options)
    %x{
      if (case_options_have_ascii(options)) {
        return self.replace(/[a-z]+/g, (match)=>{ return match.toUpperCase(); });
      }
