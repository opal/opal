...
platform.proc_is_primary = ()=>not_available("Kernel.fork");
platform.proc_is_worker = ()=>not_available("Kernel.fork");
platform.proc_fork = ()=>not_available("Kernel.fork");
platform.proc_worker_pid = (_worker)=>not_available("Kernel.fork");
platform.proc_exec = (cmd)=>os.exec([cmd.toString()], { block: true, usePath: true });
...
platform.io_popen = function(cmd, args, options) {
  let handle = os.popen(cmd.toString(), "r"), fd;
...
platform.io_fdatasync = (fd)=>handles[fd].flush();
platform.io_fstat = (_fd)=>not_available("File#fstat");
platform.io_fsync = (fd)=>handles[fd].flush();
...
platform.io_read = function(fd, io_buffer, buffer_offset, pos, count) {
  let handle = (fd == 0) ? std.in : handles[fd],
      read_buffer = new ArrayBuffer(count), len;
...
class FileStat {
  ...
  isFile() {
    return (this.obj.mode & (os.S_IFBLK | S_IFCHR | os.S_IFDIR | os.S_IFIFO | os.S_IFSOCK | os.S_IFLNK)) == 0;
  }
...
platform.file_is_absolute_path = (file_name)=>{
  let real_path, err;
  file_name = file_name.toString();
  real_path, err = std.realpath(file_name);
...
platform.file_lstat = (file_name)=>{ return new FileStat(os.lstat(file_name.toString())); }
...
platform.file_mkfifo = (file_name, mode)=>{
  ...
  os.exec(['mkfifo', '-m', mode.toString(8), file_name.toString()], { block: true, usePath: true });
  return res.status;
}
platform.file_readlink = (path_name)=>os.readlink(path_name.toString());
platform.file_realpath = (path_name, _sep)=>{
  let path, err;
  path, err = os.realpath(path_name.toString());
  return path;
}
platform.file_rename = (old_name, new_name)=>os.rename(old_name.toString(), new_name.toString());
platform.file_stat = (file_name)=>{ return new FileStat(os.stat(file_name.toString())); }
...
platform.dir_home = (_sep)=>platform.env_get("HOME");
...
platform.dir_wd = (sep)=>os.get_cwd();
