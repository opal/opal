  ...
  Opal.queue(async function () {
    ...
    function action(action, ...args) {
      try {
        ...
        let code = error.code;
        if (code === "ERR_SYSTEM_ERROR" && error.info?.code) code = error.info.code;
    ...
    // Some constants
    platform.PRIO_PGRP = 0;
    platform.PRIO_PROCESS = 1;
    platform.PRIO_USER = 2;
    ...
    if (fs.fchmodSync) {
      platform.fchmod = (fd, mode) => action(fs.fchmodSync, fd, mode);
    }
    if (fs.fchownSync) {
      platform.fchown = (fd, uid, gid) => action(fs.fchownSync, fd, uid, gid);
    }
    ...
    platform.getpriority = (_kind, pid) => os.getPriority(pid);
    platform.getuid = typeof process.getuid === "function" ? process.getuid : () => -1;
    platform.initgroups =
      typeof process.initgroups === "function" ? (user, gid) => action(process.initgroups, user, gid) : () => [];
    ...
    platform.setpriority = (_kind, pid, prio) => action(os.setPriority, pid, prio);
    platform.setproctitle = (title) => {
      process.title = title;
    };
    ...
    class Pipe {
      ...
      eof(fd) {
        if (fd === this.rfd) {
          return this.data_view.byteLength === 0;
        }
        return false;
      }
      get read_fd() {
    ...
    class SpawnPipe {
      constructor(buff_size, cmd, args, mode, options) {
        ...
        if (mode == "r") {
          ...
          function process_data(data) {
            if (!data) return;
            let i = 0,
              uint8ary = pipe.tencoder.encode(data.toString("utf8"));
          ...
          this.chpr.on("exit", function (_code) {
            pipe.closed = true;
          });
        }
      }
      ...
      eof(_fd) {
        return this.write_pos < 1;
      }
      close(_fd) {
        if (!this.closed) {
          this.closed = true;
          try {
            if (this.chpr) this.chpr.kill();
          } catch {}
          try {
            fs.closeSync(this.pfd);
          } catch {}
    ...
    platform.io_fdatasync = (fd) => {
      if (fd > 2) action(fs.fdatasyncSync, fd);
    };
    platform.io_fstat = (fd) => action(fs.fstatSync, fd);
    platform.io_fsync = (fd) => {
      if (fd > 2) action(fs.fsyncSync, fd);
    };
    platform.io_open = (fd) => {
      let tty = false,
        pipe = false,
    ...
    platform.io_read = (fd, io_buffer, buffer_offset, pos, count, is_file) => {
      ...
      while (true) {
        try {
          return action(fs.readSync, fd, io_buffer.data_view, buffer_offset, count);
        } catch (e) {
          if (e instanceof Opal.Errno.EAGAIN) {
            if (Opal.gvars["!"] === e) {
              if (Opal.exceptions.length > 0) Opal.gvars["!"] = Opal.exceptions.pop();
              else Opal.gvars["!"] = Opal.nil;
            }
            platform.sleep(0.01); // 10ms
          } else throw e;
        }
      }
    };
    ...
    platform.dir_open = (dir_name) => {
      let handle = action(fs.opendirSync, dir_name),
        fd = ++directories.last;
    ...
    platform.load_file = function (normalized_path) {
      ...
      let ruby = action(fs.readFileSync, normalized_path).toString(),
        ...
        path;
      for (path of requires) {
        platform.load_file(path, Opal.normalize(path));
      }
      eval(js);
    };
  });
