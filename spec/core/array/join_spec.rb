...
describe "Array#join" do
  ...
  pending "calls #to_str to convert the separator to a String" do
    sep = mock("separator")
    sep.should_receive(:to_str).and_return(", ")
    [1, 2].join(sep).should == "1, 2"
  end

  pending "does not call #to_str on the separator if the array is empty" do
    sep = mock("separator")
    sep.should_not_receive(:to_str)
    [].join(sep).should == ""
  end

  pending "raises a TypeError if the separator cannot be coerced to a String by calling #to_str" do
    obj = mock("not a string")
    lambda { [1, 2].join(obj) }.should raise_error(TypeError)
  end

  pending "raises a TypeError if passed false as the separator" do
