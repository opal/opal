    ...
    class TopNode < ScopeNode
      ...
      def compile
        ...
        if compiler.es_six_imexable?
          m_name = module_name(compiler.file)
          opening(m_name)
        end
        in_scope do
        ...
        if compiler.es_six_imexable?
          import_lines = compiler.requires.map do |module_path|
            ...
            unless module_path == 'corelib/runtime'
              i_line = i_line + "if (typeof Opal.modules[#{module_path.inspect}] === 'undefined') { #{i_name}(); }\n"
            end
            i_line
          end
      ...
      def module_name(filenamepath, original_filepath = nil, original_filename = nil)
        ...
        if Opal.paths.include?(path.realpath.to_s)
          return o_s[(path.realpath.to_s.size+1)..(o_s.end_with?('.rb') ? -4 : -1)]
        end
        if path.root?
          pwd = Dir.pwd
          if o_s.start_with?(pwd)
            return o_s[(pwd.size+1)..(o_s.end_with?('.rb') ? -4 : -1)]
          else
            return o_s
          end
        end
        return module_name(path, original_filepath, original_filename)
      end
        ...
        directory_path.each_child do |child_path|
          if child_path.directory?
            ...
            if path_s.end_with?('.rb') || path_s.end_with?('.js')
              ...
              import_lines << "import '#{module_path}';\n"
            end
          end
        end
      end
