...
module Opal
  module Nodes
    class CallNode < Base
      ...
      def recvr_is_const_named?(const_name)
        recvr && recvr.type == :const && begin
          ...
          if const_name.to_s.include? "::"
            const_base_name, const_name = const_name.to_s.split("::").map(&:to_sym)
          end

          if const_base_name
            const_base && const_base.type == :const && const_base.children[1] == const_base_name
          else
      ...
      # For use inside handle_special - use this helper to convert the arguments
      # given to block arguments.
      def argumentize(args = arglist, perhaps_use: nil)
        argumentized = args.children.map do |arg|
          argumentize_single(arg, perhaps_use: perhaps_use)
        end
        ...
        if block_given?
          ...
          yield(*argumentized, **kwargs)
        else
          argumentized
        end
      end

      def argumentize_single(arg, perhaps_use: nil)
        case arg.type
        ...
        when :false
          false
        when :array
          argumentize(arg, perhaps_use: perhaps_use)
        when :hash
          arg.children.map do |pair|
            error "Only pairs are supported" unless pair.type == :pair
            [
              argumentize_single(pair.children[0], perhaps_use: perhaps_use),
              argumentize_single(pair.children[1], perhaps_use: perhaps_use)
            ]
          end.to_h
        else
          error_msg = "Type #{arg.type} is not supported for an argumentized macro"
          error_msg += " (perhaps use: #{perhaps_use})" if perhaps_use
          error error_msg
        end
      end
