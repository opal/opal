...
class Opal::RackHandler
  ...
  def initialize(app, options = {})
    ...
    if @hot_updates
      ...
      if hot_ruby
        @hot_javascript = Opal::Compiler.new(hot_ruby, requirable: false, file: '(hot_ruby)').compile
      else
  ...
  def call_js(path, rest)
    ...
    [
      200,
      { 'content-type' => 'application/javascript', 'last-modified' => asset[:mtime] },
  ...
  def poll_updates
    ...
    if ((updates[:modified].size + updates[:added].size) > 5) || updates[:removed].any?
      ...
      # compile changes
      updates[:modified].each do |asset|
        if asset.is_a?(Opal::Builder::Processor::RubyProcessor)
          data[:modified] ||= []
          data[:modified] << asset.source
        end
      end
      updates[:added].each do |asset|
        if asset.is_a?(Opal::Builder::Processor::RubyProcessor)
          data[:added] ||= []
          data[:added] << asset.source
        end
      end
    end
