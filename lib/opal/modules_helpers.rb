  ...
  class ModulesHelpers
    module ES6ImportHelpers
      ...
      def generate_directory_imports(base_dir, tree_path = nil)
        ...
        import_lines = []
        base_dir = Opal::ModulesHelpers.determine_real_module_dir(base_dir)
      ...
      def generate_module_import(ruby_module_path)
        ...
        if absolute_module_path.end_with?('.js')
          import_lines << "import #{import_name} from '#{absolute_module_path}';\n" # delegate to webpack .js loader with absolute path
        else
          import_lines << "import #{import_name} from '#{import_module_name}';\n" # delegate to opal-webpack-loader
        end
    ...
    class << self
      ...
      def absolute_module_path(module_path)
        ...
        raise "Could not resolve module '#{module_path}'! Known load paths:\n #{Opal.paths.join("\n")}\n"
      end
