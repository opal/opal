...
module Opal
  class AppCreator
    ...
    def initialize(type, src_out)
      raise "Cannot create app: unsupported application type '#{type}'" unless APP_TYPES.include?(type)
      if src_out.instance_of?(IO)
        @out_path = "opal_#{type}_app"
      elsif src_out.is_a?(File)
        @out_path = src_out.path
      else
        raise 'Cannot create app: source must be a file.'
      end
    ...
    def create_osascript_app(src_path, _dir)
